...
export default async function handler(req, res) {
  ...
  let enviados = 0, muertos = [], resumenLimpieza = null;
  if (VAPID_PUBLICA && VAPID_PRIVADA) {
    ...
    /* Envuelto (01-09-2026, barrido): con el almacén suspendido esta
       línea LANZA, y como el parte ya se ha enviado arriba, lo que
       revienta es la respuesta — Vercel devuelve 500 y desde fuera el
       parte parece no haber salido cuando sí salió. Es el mismo arreglo
       que ya se hizo en `avisar.mjs` y en `empujar()` del vigilante.
       Una limpieza de aparatos de baja no puede tumbar un envío hecho. */
    let noSeLimpio = null;
    if (muertos.length) {
      try { await guardar(aparatos.filter(a => !muertos.includes(a.endpoint))); }
      catch (e) { noSeLimpio = String(e?.message || e).slice(0, 80); }
    }
    if (noSeLimpio) resumenLimpieza = noSeLimpio;
  }
  ...
  return res.status(200).json({ ok: true, titulo: tit, cuerpo, enviados,
                                ...
                                /* Si la limpieza de aparatos de baja no se pudo
                                   guardar se DICE, en vez de tumbar el envío que
                                   ya se ha hecho (01-09-2026). */
                                noSeLimpio: resumenLimpieza || undefined,
                                vigilante });
}
