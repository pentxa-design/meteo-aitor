...
function fixedValenciaCameras() {
  return VALENCIA_CAMERAS.map(([key, name, streamId, originalUrl], index) => ({
    id: index + 1,
    key,
    name,
    optionLabel: name,
    ...
    status: 'Vista oficial actualizada',
    originalUrl,
...
function fixedAmetllaCameras() {
  ...
  return AMETLLA_CAMERAS.map(([key, name, videoId], index) => ({
    id: index + 1,
    key,
    name,
...
function fixedCadizCameras() {
  return CADIZ_CAMERAS.map(([key, name, videoId, originalUrl, sourceLabel], index) => ({
    id: index + 1,
    key,
    name,
    optionLabel: `${name} · directo`,
...
module.exports = async function webcamsHandler(req, res) {
  ...
  const [galiciaResult, cataloniaResult] = await Promise.allSettled([loadMeteoGalicia(), loadThreeCat()]);
  ...
  const catalogs = {
    ...
    valencia: catalog('valencia', 'Comunitat Valenciana · Turisme CV', 'Turisme Comunitat Valenciana', 'https://www.comunitatvalenciana.com/es/webcams', fixedValenciaCameras()),
