...
export function cabeceras(ttlSegundos, opciones = {}) {
  ...
  const {
    ...
    cors = true,
    origen = null,
    /* cdn: false → el navegador guarda `navegador` segundos y el CDN NADA.
       Para los trozos con Range del mapa (14-09-2026): un 206 no se cachea y
       una copia entera guardada rompería los rangos siguientes. */
    cdn = true,
  } = opciones;
  ...
  if (ttl > 0 && !cdn) {
    h['cache-control'] = `private, max-age=${entero(navegador, 60)}`;
