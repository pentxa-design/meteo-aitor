...
const LAST_GOOD_TTL = OBSERVATION_MAX_AGE;
...
function fetchOfficial(path, { accept, timeout, maxBytes }) {
  return new Promise((resolve, reject) => {
    ...
    const request = https.get({
      ...
      headers: {
        Accept: accept,
        'User-Agent': 'MeteoAitorBermeo/10.130 (+https://meteoaitorbermeo.vercel.app)'
      }
    }, response => {
...
async function directoryCandidates(product, rounded) {
  ...
  }).filter(item => item.ageMinutes <= OBSERVATION_MAX_AGE / 60000)
...
module.exports = async function handler(req, res) {
  ...
  // atraviesa una incidencia se busca como máximo una hora atrás y se identifica el
  ...
  if (!frame) {
    ...
    return res.status(503).json({
      ...
      error: 'Euskalmet no publicó un fotograma utilizable durante la última hora.'
    });
  }
