...
async function selectRun(bounds, resolution, now = Date.now()) {
  ...
  for (const run of candidateRuns(now)) {
    try {
      // La primera página solo depende de +3…+24 h. No se bloquea su entrega
      ...
      // Si cualquiera de los ocho intervalos visibles falta, se prueba una
      ...
      if (frames.some(frame => !frame)) throw new Error('NOAA aún no completó la primera página (+3…+24 h) de esta pasada.');
      return {
        run,
