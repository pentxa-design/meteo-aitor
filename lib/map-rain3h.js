...
function gridResolution(requestedBounds) {
  ...
  return latitudeSpan > REGIONAL_LATITUDE_SPAN || longitudeSpan > REGIONAL_LONGITUDE_SPAN
    ? { code: '1p00', degrees: 1, overview: true }
    : { code: '0p25', degrees: 0.25, overview: false };
}

function canonicalRegionalBounds(center) {
  // Ventana estable de 18° × 30°. Mantiene toda la Península y buena parte de
  // Francia en una sola consulta y reutiliza caché al hacer paneos pequeños.
  const latitude = Math.max(-71, Math.min(76, Math.round(center.latitude / 2) * 2));
  const longitude = Math.max(-164.75, Math.min(164.75, Math.round(center.longitude / 2) * 2));
  return {
    south: latitude - REGIONAL_LATITUDE_SPAN / 2,
    north: latitude + REGIONAL_LATITUDE_SPAN / 2,
    west: longitude - REGIONAL_LONGITUDE_SPAN / 2,
    east: longitude + REGIONAL_LONGITUDE_SPAN / 2
  };
}
...
function outputGrid(bounds, resolution) {
  const rows = resolution.overview
    ? 48
    : Math.round((bounds.north - bounds.south) / resolution.degrees) + 1;
  const columns = resolution.overview
    ? 96
    : Math.round((bounds.east - bounds.west) / resolution.degrees) + 1;
  const points = [];
...
async function selectRun(bounds, resolution, now = Date.now()) {
  ...
  for (const run of candidateRuns(now)) {
    ...
    const startHour = Math.floor(elapsedHours / FRAME_STEP_HOURS) * FRAME_STEP_HOURS;
    const finalHour = startHour + FORECAST_SPAN_HOURS + FRAME_STEP_HOURS;
    try {
      const [first, final] = await Promise.all([
        startHour > 0 ? fetchCumulativeFrame(run, startHour, bounds, resolution) : Promise.resolve(null),
        fetchCumulativeFrame(run, finalHour, bounds, resolution)
      ]);
      const probes = new Map();
      if (first) probes.set(startHour, first);
      probes.set(finalHour, final);
      return { run, startHour, probes };
    } catch (error) {
      lastError = error;
    }
  }
...
function compactPoint(point, intervals) {
  ...
  return {
    ...
    hourly: { precipitation_3h: precipitation }
  };
}
...
function trimCache() {
  ...
  while (RESPONSE_CACHE.size > 6) RESPONSE_CACHE.delete(RESPONSE_CACHE.keys().next().value);
}
...
module.exports = async function handler(req, res) {
  ...
  const bounds = resolution.overview ? { ...WORLD_BOUNDS } : canonicalRegionalBounds(viewportCenter);
  ...
  try {
    ...
    const offsets = Array.from(
      { length: Math.floor(FORECAST_SPAN_HOURS / FRAME_STEP_HOURS) + 2 },
      (_, index) => selected.startHour + index * FRAME_STEP_HOURS
    );
    const cumulative = await fetchCumulativeFrames(selected.run, offsets, noaaBounds, resolution, selected.probes);
    const intervals = buildIntervals(cumulative);
    if (intervals.length < 3) throw new Error('NOAA no devolvió suficientes intervalos consecutivos de lluvia 3 h.');
    const times = intervals.map(interval => new Date(selected.run.getTime() + interval.startHour * HOUR_MS).toISOString().replace(/:00\.000Z$/, ':00'));
    ...
    const payload = {
      ...
      diagnostics: {
        ...
        derived: true,
        sourceVariable: 'APCP',
        interval: 'difference-of-consecutive-official-cumulative-fields',
        intervalHours: 3,
        units: 'mm'
      },
      model: {
        ...
        label: resolution.overview
          ? 'NOAA GFS · lluvia 3 h · fuente 1° · vista mundial reducida'
          : 'NOAA GFS 0,25° · lluvia 3 h oficial',
    ...
    return res.status(502).json({
      ...
      source: 'NOAA NOMADS',
      retryAfter: 120,
      error: `NOAA GFS no pudo servir ahora la lluvia exacta de 3 horas. ${String(error?.message || '')}`.trim()
    });
  }
};
