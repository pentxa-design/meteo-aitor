...
// La primera imagen necesita solo cuatro acumulados (+3…+12 h). Reducir cada
// página a 12 h divide a la mitad las descargas GRIB de la primera carga; las
// páginas restantes completan +168 h en segundo plano sin bloquear el mapa.
const PAGE_HOURS = 12;
...
async function selectRun(bounds, resolution, now = Date.now()) {
  ...
  for (const run of candidateRuns(now)) {
    try {
      // La primera página solo depende de +3…+12 h. No se bloquea su entrega
      // esperando +168 h: NOAA publica la pasada de forma progresiva y un
      // fallo transitorio del último horizonte no invalida datos ya oficiales.
      // Si cualquiera de los cuatro intervalos visibles falta, se prueba una
      // pasada anterior completa para esta página. Las páginas posteriores
      // quedan fijadas al mismo run/base y nunca mezclan ciclos.
      const probeHours = pageForecastHours(0);
      const frames = await fetchCumulativeFrames(run, probeHours, bounds, resolution, new Map());
      if (frames.some(frame => !frame)) throw new Error('NOAA aún no completó la primera página (+3…+12 h) de esta pasada.');
      return {
        run,
...
function trimCache() {
  ...
  // Catorce páginas forman una pasada completa. Se conservan dos juegos para
  // que una segunda visita no expulse la primera página mientras carga +168 h.
  while (RESPONSE_CACHE.size > 30) RESPONSE_CACHE.delete(RESPONSE_CACHE.keys().next().value);
}
...
module.exports = async function handler(req, res) {
  ...
  if (cached && Date.now() - cached.savedAt < CACHE_TTL) {
    res.setHeader('Cache-Control', 's-maxage=1200, stale-while-revalidate=900');
