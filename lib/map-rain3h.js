...
function gridResolution(requestedBounds) {
  ...
  if (latitudeSpan > WIDE_LATITUDE_SPAN || longitudeSpan > WIDE_LONGITUDE_SPAN) {
    return { code: '1p00', degrees: 1, displayDegrees: 3.75, overview: true, wide: false };
  }
...
function outputGrid(bounds, resolution) {
  const rows = resolution.overview
    ? 48
    : Math.round((bounds.north - bounds.south) / resolution.displayDegrees) + 1;
  const columns = resolution.overview
    ? 96
    : Math.round((bounds.east - bounds.west) / resolution.displayDegrees) + 1;
  const points = [];
    ...
    const payload = {
      ...
      model: {
        ...
        label: resolution.overview
          ? 'NOAA GFS · precipitación oficial · fuente 1° · vista mundial reducida'
          : resolution.wide
