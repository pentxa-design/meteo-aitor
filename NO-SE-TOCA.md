...
| **Las pantallas se arrancan enteras ANTES de publicar, a las 02, 07, 13 y 20 h, con datos trampa distintos en cada hora: la mar de la portada, la luna contra el orto, la chapa de racha, el «lo ve», el pie de estaciones, el marcador y nada de NaN ni decimales con punto (25-09-2026: seis fallos de pantalla en sus pantallazos de las 07:13; `pantallas.cjs`, visto en rojo rompiendo cada arreglo sobre una copia)** | `revisar.sh arranca pantallas.cjs y deploy.sh no publica si una pantalla dice lo que no toca` |
| **La chapa «Racha» de la tarjeta de la HORA se colorea por su listón, no por el semáforo de la hora (25-09-2026, misma familia que la del día)** | `la chapa «Racha» de la tarjeta de la hora lleva SU color (nivelRacha), no el semáforo de la hora` |
| **Ninguna pantalla dice «aguanta» cuando la pareja de 700/75 salta: la palabra de la escala solo se alcanza por `colaTapa()` / `colaTapaLarga()`, que preguntan la pareja antes (25-09-2026 en Horas, Detalles y Mis estaciones; 26-09-2026 en la PORTADA y en la ficha de índices, que se habían quedado fuera)** | `ninguna pantalla imprime la palabra de la tapa sin haber preguntado por la pareja` |
| **Y el mismo hecho del mismo sitio y la misma hora dice lo mismo en Ahora, Horas y Mis estaciones: racha, viento, CAPE, cifra de la tapa, lo que hace la tapa, lluvia, temperatura y cielo (26-09-2026; `pantallas.cjs`, visto en rojo con tres sabotajes distintos)** | `revisar.sh arranca pantallas.cjs y deploy.sh no publica si una pantalla dice lo que no toca` |
