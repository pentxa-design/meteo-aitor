...
import { get, put, del } from './blob-falso.mjs';
...
/* Las cuatro opciones que llevaban los trece sitios, ahora en uno.
   `allowOverwrite` porque estos cajones se pisan a propósito: cada
   guardado sustituye al anterior. `addRandomSuffix: false` porque la
   ruta ES el nombre del cajón; con sufijo no lo volveríamos a
   encontrar. `private` porque aquí dentro hay sus avisos y sus notas
   del monte, y eso no se sirve abierto a internet. */
...
export const guardarJSON = (ruta, dato) =>
  put(ruta, JSON.stringify(dato), { ...COMUN, contentType: 'application/json' });

/** Guarda bytes tal cual: una foto suya del monte, o un día comprimido.
 *  `tipo` es el content-type ('image/jpeg', 'application/gzip'…). */
export const guardarBytes = (ruta, bytes, tipo) =>
  put(ruta, bytes, { ...COMUN, contentType: tipo });
...
export const borrar = async rutas => {
  try { await del(rutas); return true; } catch { return false; }
};
