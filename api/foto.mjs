...
import { leerLista, leerBytes, guardarJSON, guardarBytes, borrar } from '../lib/almacen.mjs';
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      const bytes = await leerBytes(ruta);
      if (!bytes) return res.status(404).json({ error: 'la foto ya no está' });
  ...
  try {
    await guardarBytes(ruta, bytes, 'image/jpeg');
    ...
    if (fuera.length) await borrar(fuera);   // si falla, se reintenta la próxima

    await guardarJSON(CAJON, quedan);
