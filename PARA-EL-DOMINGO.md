...
- **10-09-2026, 11:12, build 1112: el mapa, de las capturas de Aitor a las 11:05.** (1) **Números de otra hora bajo el sello de esta.** En `setT()` (cambio de hora sin rehacer la capa) se cambiaban las teselas y se programaba `valores()` a los 500 ms, pero los números y las barbas de la hora ANTERIOR seguían en pantalla hasta que la nueva estaba descodificada (17-24 s con ECMWF HRES). Dos capturas de «jue 14:00» tenían Argel al 77 % y al 37 %; la API de ECMWF HRES daba 39 → la segunda era la buena, la primera era de otra hora. Además dos cambios seguidos podían acabar con la pasada VIEJA pintando después de la nueva. Arreglo: `limpiarValores()`/`limpiarBarbas()` en el acto en `setT()`, y un turno (`_turnoVal`, `_turnoBarb`, `vigente()`) en `valores()`/`barbas()` para que solo pinte la última llamada y solo si la URL sigue siendo la de la hora/capa/modelo actual. Comprobado en Pages: al cambiar de hora los números desaparecen al instante y vuelven a los ~8 s; el 73 % que pinta ICON-EU en (38.785, −0.194) a las 12:00 es el 74 % de la API. Si el mapa no tiene tamaño (candado, pestaña oculta) ya no pinta seis etiquetas iguales en (0,0). (2) **RainViewer solo sirve hasta z7.** Medido con curl: en 256 y 512 px, de z8 en adelante devuelve una imagen «Zoom Level Not Supported». `radarSrc` (Radar observado y Radar + previsión) tenía `maxzoom:12`, así que al acercarse a Calpe pedía z8-z9 y el radar salía en blanco o con cajitas. Ahora `RAINVIEWER_ZMAX = 7` (junto a `TILES_PROXY`) y el mapa estira la de z7; comprobado a z9,2 sobre Calpe: solo pide z7. El Radar de la pestaña (Leaflet, app.js) ya tenía `maxNativeZoom: 7`. (3) **Radar AEMET «Failed to fetch»** a las 11:02 con el servidor respondiendo en 0,2 s: `applyAemet()` reintenta una vez a los 1,5 s antes de rendirse. (4) **«El mapa se había quedado sin memoria y se ha recargado solo»** con ECMWF HRES + Nubes + lluvia + Valores + Rayos: no se ha tocado; el gestor de contexto perdido ya recarga. Para animar, mejor ECMWF 25 km o ICON-EU. **Del tiempo:** día gris con llovizna en Calpe; AROME falló dos veces (16 mm a las 8 que no cayeron, despeje a las 11 que no llegó) y GFS acertó. Lección para el veredicto de las franjas: cuando GFS/ECMWF ven llovizna y el modelo cargado la ve seca, el titular debería ser duda («puede lloviznar»), no sol.

- **10-09-2026, 11:40, build 11xx: EL BUCLE DE RECARGA, DE RAÍZ.** Al probar el build 1117 en el Mac la app se recargó varias veces seguidas, y la causa no era un sello a medias: GitHub Pages sirve app.js/maps.js/styles.css con `cache-control: max-age=600`, `comprobarVersion()` pide version.json con `no-store` (siempre fresco), y `location.reload()` NO revalida los scripts: el navegador se queda hasta 10 minutos con el app.js viejo. Encima el service worker (red primero) hacía `fetch(request)` con la petición tal cual, cogía ese app.js viejo de la caché HTTP y lo guardaba en la caché nueva `torre-<build>`. Resultado: version.json nuevo + BUILD viejo → recarga → igual → recarga… hasta que caduca la caché HTTP. Es, casi seguro, el «parpadeo» del iPhone del 07-09 (aquel día además había sello a medias). Tres arreglos: (1) sw.js: el casco (`/`, index.html, styles.css, app.js, maps.js, version.json) se pide con `cache:'no-cache'` (revalida con ETag: 304 si no cambió, cuesta lo mismo); (2) app.js `comprobarVersion()`: antes de recargar, `fetch(f, {cache:'reload'})` de los cuatro ficheros del casco, que refresca la caché HTTP y, a través del SW, la del build; (3) red de seguridad: a la tercera recarga por versión en 3 minutos se para y avisa «No consigo actualizar a X (sigo en Y). Cierra la app del todo y vuelve a abrirla». **En casa (Vercel) comprobar qué cache-control mandan app.js y version.json** (`curl -sI`): si app.js va con max-age largo, el mismo bucle puede darse allí tras cada deploy; los tres arreglos valen igual.

- **10-09-2026, 13:35: EL VIGILANTE DEL SERVIDOR LLEVA PARADO DESDE LAS 06:00.** El iPhone de Aitor enseñaba «⚠ NADIE ESTÁ VIGILANDO · 7 h 30 min». Comprobado desde el portátil contra Vercel: `GET /api/vigilante?pulso=1` → `{"ultima":"2026-09-10T04:00:44Z","haceMin":451,"envia":true,"parteDe":"2026-09-10","sitios":20,"revivido":true}`. O sea: la última pasada fue a las 06:00 hora local y no ha vuelto a pasar. Según app.js lo lanza cron-job.org cada media hora (no depende del Mac de casa). Sospechosos, por orden: (1) el trabajo de cron-job.org en pausa o fallando (mirar su historial de ejecuciones y el código HTTP que recibe); (2) la función `/api/vigilante` cayendo o agotando el tiempo en Vercel (Logs del proyecto, filtrar por vigilante, desde las 04:00Z); (3) el sello «ultima» no se escribe aunque la pasada corra («revivido: true» huele a KV o almacén que se reinició: comprobar qué significa en api/vigilante). Mientras esté parado NO salen avisos push aunque una torre entre en umbral; las tarjetas de Mis estaciones sí son frescas porque las pide el propio móvil («Datos pedidos hoy a las 13:30 · los 20 emplazamientos a la vez»). Aitor: «me preocupa lo del vigilante», «el domingo habrá que mirar todo». Va junto con la sección 11 (avisos que gritan por todo).

- **10-09-2026, 19:00, más sobre el vigilante: PASA PERO NO APUNTA, Y AVISA EN BUCLE.** A las 18:55 el iPhone recibió el push «⚠ He estado 12 h sin vigilar · Mi última pasada fue hace 12 h 55 min… Ahora vuelvo a estar», y a las 18:57 y 18:58 dos más iguales («12 h 57», «12 h 58»). O sea: el vigilante SÍ corre (manda push), pero el sello `ultima` no se escribe (el pulso seguía en 04:00Z a las 18:57 y a las 19:00), así que cada pasada cree que lleva 12 h parada y vuelve a mandar el mismo aviso. Sospecha fuerte: `?pulso=1` (que la app llama al abrirse y yo llamé cada 10-15 s desde el portátil entre 18:57 y 19:00) «revive» el vigilante y dispara una pasada (`revivido: true`); las pasadas coinciden con esas llamadas. He dejado de llamar al pulso desde el portátil a las 19:00 para no seguir disparando avisos a Aitor. Para el domingo, en api/vigilante: (1) por qué no se escribe `ultima` (KV/almacén: ¿permisos, clave cambiada, escritura que falla en silencio?); (2) el aviso «he estado X h sin vigilar» debe salir UNA vez por resurrección, no en cada pasada; (3) el pulso no debería disparar pasadas (o al menos no más de una cada 30 min); (4) revisar cron-job.org (historial y respuesta).

- **10-09-2026, 19:10: «Radar AEMET no va» en el Mac, y era la app de localhost.** La app instalada en el Mac (`~/Applications/Chrome Apps.localized/Aitor Meteo…app` → `http://localhost:8767/index.html`, servida por `.tools/servir.js`) pedía la imagen del radar DIRECTA a aemet.es porque `AEMET_RADAR.via()` solo usaba el intermediario fuera de localhost/127.0.0.1/192.168. aemet.es no manda CORS, así que el navegador no deja leer los píxeles y `fetch` lanza «Failed to fetch» (11:02 y 19:05, con Vercel contestando en 0,2 s). En Pages funcionaba (comprobado a las 19:06 en el navegador: «eco de lluvia en el <0,1 % del territorio»). Arreglo: `via()` usa el intermediario siempre que la app venga de un servidor http/https (en el Mac servir.js lo reenvía a Vercel; en Pages BACKEND es Vercel; en Vercel es el propio origen) y solo va directo si se abre como file://. En casa no cambia nada (mismo origen), pero conviene llevarlo para que cualquier copia local funcione.

- **10-09-2026, 19:15, del repaso de las siete capturas del Mac (Ahora, 10 días, Mar):** (1) **Mar:** el rótulo decía «no se muestra una estimación del modelo» con la curva del nivel del mar del modelo dibujada justo encima. Ahora el texto dice la verdad: la curva es el nivel del mar según el modelo (vale para ver si sube o baja, no para la hora exacta) y lo que no se calcula fuera de la costa vasca es la tabla de pleamares y bajamares. (2) **Dirección del viento:** la tarjeta ponía «De donde viene el viento a 80 m» junto a «Viento a 10 m» y «Ráfaga a 10 m». `horaDe()` cogía siempre la dirección de 80 m si existía. Ahora la dirección va al nivel del viento que se enseña: 10 m a pie de caseta, 80 m si la altura pedida es ≥ 40 m (lo alto de la torre); campo nuevo `dirNivel` para el rótulo. (3) **10 días, «Hoy · 100 % · 17,4 mm»:** es el acumulado del MODELO (AROME) para hoy, con los 16 mm del chaparrón de las 8 que nunca cayó (el radar no lo vio). No es un fallo de la app, pero para «hoy» convendría algún día distinguir lo caído (medido) de lo previsto; apuntado, no tocado. Lo demás cuadra: comparativas de nubes, tormenta (ninguno ve tormenta, tapa puesta), ráfaga (15-28 km/h, listón 49/70), lluvia (los 5 secos, 24 h secas), riesgo eléctrico, mareas.

- **11-09-2026, 10:50, del repaso de doce capturas del Mac (Mar, Avisos, 10 días, Horas, Ahora entero):** todo cuadra con el cielo real (raso, cúmulos de buen tiempo en la sierra) y entre pestañas. Dos textos corregidos: (1) «apta ahora y hasta las 09:00 **de el** domingo» → `deDia()` contrae el artículo («del domingo»); (2) la cabecera de Nubes decía «Coinciden en el cielo, con poca diferencia» con 41 puntos entre ICON (41 %) y el europeo (0 %): el corte era ≥ 50 = no coinciden, resto = poca diferencia. Ahora hay tres niveles: < 20 coinciden, 20-49 «Se parecen, sin ser iguales · hay N puntos», ≥ 50 no coinciden. Apuntado sin tocar: con altura 10 m las tarjetas «Ráfaga estimada a 10 m ×1,00» y «Viento estimado a 10 m» repiten las de al lado; se podrían ocultar cuando la altura pedida es la publicada.

- **12-09-2026, 00:45, del repaso de catorce capturas de madrugada (Mar, Ahora entero, 10 días, Horas):** cuatro textos que decían una cosa con el número diciendo otra. (1) Detalles → Sensación «24 °C · Como marca el termómetro» con el aire a 22: el rótulo solo cambiaba a partir de 3°; ahora dice la diferencia desde 1° («2° más que el aire») y el porqué desde 3°. (2) Sol y aire → Periodo «mar de viento: corta y picada» con ola de 0,3 m y, al lado, «Mar de viento 0,0 · la mar está limpia»: por debajo de 0,5 m ahora dice «mar rizada: apenas nada». (3) Mar de fondo «por debajo de la del viento» con fondo 0,3 y viento 0,0: ahora compara de verdad («y es la que manda» / «casi nada»). (4) Tormenta, cabecera «publican la tapa: sigue puesta con la gasolina que hay» con la tapa en 18 (abierta) y CAPE 130: ahora distingue «está abierta, pero no hay gasolina: CAPE máximo 130, y hacen falta 700» de «sigue puesta». Lo demás cuadra (10 días con el chip «ICON da 50 km/h, tu listón es 49» el viernes 18; ventana «hasta las 23:00 de mañana domingo»; ráfagas 9-16; nubes 41 puntos con el texto nuevo). **Pregunta abierta para Aitor:** a las 00:35 las franjas son Mañana 6-13 / Tarde 14-20 / Noche 21-23 de hoy y las horas 1-5 no tienen franja (no existe «Madrugada»). Si alguna vez trabaja de noche, convendría una franja «Madrugada · 0-5 h» cuando es de noche.

- **12-09-2026, 00:55: FRANJA «MADRUGADA · 0-5 h».** Aitor: *«trabajo de noche casi siempre»*. Las franjas eran Mañana 6-13 / Tarde 14-20 / Noche 21-23 y las horas 0-5 no tenían resumen. `bands` en `renderParts` lleva ahora `['Madrugada', 0, 5]` delante; el resto (orden por hora, salto al día siguiente cuando la de hoy ya pasó, luna por `deDia`) vale igual. Son cuatro tarjetas; `.parts` es `auto-fit minmax(140px)` y caben. Ojo en casa: si el vigilante o el parte del servidor agrupan por franjas, añadir la madrugada también allí.
...
- **13-09-2026, 18:55, logs de Vercel (weather-app, última media hora):** `/api/vigilante` entra cada ~5 min (18:30:33, 18:35:32, 18:39:50, 18:52:19). La de 18:52:19 era `curl/8.7.1` con `?pulso=1` recibida en cdg1 (París) → seguramente el propio chat del iMac comprobando el pulso, seguida de `HEAD /version.json` y `HEAD /app.js` (paso 2 del guion). **El almacén de estado del vigilante es Upstash Redis** (`POST optimum-slug-146361.upstash.io`, integración Upstash del equipo), no Blob. Hipótesis fuerte para «pasa pero no apunta»: el plan gratuito de Upstash tiene tope DIARIO de comandos; el cron de las 04:30Z (`/api/parte`) escribe el sello porque el día acaba de empezar y hay cuota; a lo largo del día las pasadas cada 5 min más las lecturas de la app la agotan y las escrituras fallan en silencio. Comprobar en la consola de Upstash (consola.upstash.com, cuenta de Aitor) el uso diario y los errores `ERR max daily request limit exceeded`. Los 3-4 «errores» de los logs de Vercel son solo `DeprecationWarning: url.parse()`, no fallos. Ejecución 435 ms, 195 MB, región iad1.

- **13-09-2026, 19:05, CAUSA MÁS PROBABLE DEL VIGILANTE, vista en Vercel → Storage:** el Blob Store **`avisos-aitor`** (proyecto weather-app, creado el 26-08) está en **«Limits Exceeded»**: operaciones avanzadas **2k/2k** (agotadas), simples 8,5k/10k, 1,63 MB, carpetas `agenda/`, `avisos/`, `campo/` y `marcador-modelos.json`. Las operaciones avanzadas de Blob son `list` (y copy). Si `api/vigilante` hace `list('avisos/')` en cada pasada para saber qué avisó o cuándo pasó, desde que se agotó la cuota ese `list` falla → no encuentra la última pasada → «he estado X h sin vigilar» en cada pasada y sello sin actualizar. El Upstash Redis `almacen-aitor` (2 proyectos, Free) está «Available». Qué hacer en `api/vigilante` y `api/parte`: (1) ninguna llamada a `list` en la pasada normal: guardar el estado (última pasada, avisos enviados hoy) en UNA clave de Upstash o en UN blob con nombre fijo leído con `head`/`get` (operación simple); (2) las pasadas cada 5 min sobran: cada 30 min basta, y así también baja la CPU (7 h de 4 h); (3) cuando el ciclo se reinicie (ciclo 14-ago → 13-sept, o sea mañana) volverá a funcionar SOLO hasta que se agote otra vez si no se cambia lo del `list`. Con la cuota agotada, cualquier prueba de hoy que use `list` fallará: probar mañana o tras el cambio.

- **13-09-2026, 19:20, hecho desde el portátil con permiso de Aitor:** en Vercel → proyecto `meteo_aitor_bermeo` → Settings → Build and Deployment → Ignored Build Step, «Behavior» pasa de «Automatic» a **«Only build production»** («Ignored build step updated»). Los pushes a `portatil-2026-09-07` ya no generan previews. Y corrección al apartado 15: el chat del iMac demostró que el vigilante NO usa el Blob desde el 4 de septiembre (va a Redis/Upstash); la causa real era que el sello solo se escribía si algo cambiaba y el pulso lo «resucitaba» en cada apertura. Arreglado y publicado por el iMac en el build 1901 (sello en cada pasada, resucitada no repite en 20 min). Comprobado desde el portátil a las 19:07: «última pasada hace 5 min», sin «revivido».
