...
- **cron-job.org NO está llamando.** Medido: el sello quedó en 17:02:49Z (la pasada que resucitó mi pulso) y a las 17:42Z seguía igual: la pasada de las :30 no llegó. Hoy el vigilante solo corre cuando la app lo resucita (>4 h sin sello). Aitor tiene que mirar en cron-job.org si el trabajo está pausado/fallando y qué código HTTP recibe (con la pasada larga puede haber superado el tiempo del plan gratuito). Mientras, la tarea local `vigilante-tormentas` del Mac (cada 3 h) sigue.
